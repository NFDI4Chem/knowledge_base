...
export default function Methods({ defaultProfile }) {
	...
	const handleChange = (e) => {
		...
		setFilterProfile("");
	}; // handle text input changes in state
		...
		return (
			<button
				...
				onClick={() => {
					...
					setSearchFilter("");
				}} // Clicking button sets filter state and resets text search state
			>
				{longname}
			</button>
		);
	...
	if (searchFilter == "") {
		// decide which state to use for filtering
		...
		result = table.filter((obj) =>
			JSON.stringify(obj)
				.toLowerCase()
				.includes(searchFilter.toLowerCase()),
		); // JSON.stringify squashes table entry object for string search
		...
		<React.Fragment>
			<div className="methods__searchfilter">
				<div className="row methods__searchfilter__row">
					<div className="methods__searchfilter__filter">
						Click to filter:{" "}
						{profiles.map((props, idx) => (
							<FilterButton key={idx} {...props} />
						))}
					</div>
					<div className="col methods__searchfilter__search">
						<input
...
function TableHead({ alignment, activeHeaders }) {
	...
	return (
		<thead>
			<tr>
				{activeHeaders
					.filter((header) => header !== "shortname")
					.map((header) => (
						<th key={"header_" + header} align={alignment}>
							{headers[header]}
						</th>
					))}
			</tr>
			...
			{activeHeaders
				...
				.map((header) => {
					if (entry[header] && entry[header].toString() === "true") {
						return (
							<td key={Math.random()} align="center">
								&#10004;
							</td>
						);
					} else {
	...
	var activeHeaders =
		resultSet[0] === "all"
			...
			: Array.from(
				...
				); // Get list of headers required for found set or all headers for all

	if (found.length === 0) {
		return (
			<h4>
				No methods match your search query. Is your method missing?
				Contact us via{" "}
				<a href="mailto:helpdesk@nfdi4chem.de">helpdesk@nfdi4chem.de</a>
				!
			</h4>
		);
	}
	...
	if (resultSet[0] === "all") {
		return (
			<table>
				...
				<tbody>
					{table
						.filter((entry) => entry.shortname !== "headers")
						.map((entry, idx) => (
							<Entry
								key={"entry_" + idx}
								{...{ entry, activeHeaders }}
