...
function Lbe() {
	...
	var repos = Array.from(
		new Set(
			lbeTable.map((obj) => obj.linkdata.map((obj) => obj.name)).flat(),
		),
	).sort((a, b) => a.localeCompare(b, undefined, {sensitivity: "base"}));
	...
	if (lbeState.repo == "All" || lbeState.subd == "All") {
		...
		switch (lbeState.switch) {
			case "tag":
				...
				break;
			case "repo":
				result = lbeTable.filter((n) =>
					n.linkdata.map((n) => n.name).includes(lbeState.repo),
				);
				break;
			case "subd":
				result = lbeTable.filter((n) =>
					n.subdiscipline.includes(lbeState.subd),
				);
				break;
			case "journal":
				result = lbeTable.filter((n) =>
					n.journal.includes(lbeState.journal),
				);
				break;
			case "search":
				result = lbeTable.filter((obj) =>
					JSON.stringify(obj)
						.toLowerCase()
