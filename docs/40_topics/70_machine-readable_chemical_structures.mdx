...
## SMILES, InChI and InChIKey and Supplementary Table
...
- number of structure in the research article as local identifiers
- letter-code and number in your lab journal as sample identifiers
- SMILES
- InChI and InChIKey
...
Templates for this table are provided as [.ods](pathname://../../../assets/Supplementary_table_template.ods), [.xlsx](pathname://../../../assets/Supplementary_table_template.xlsx), and [.csv](pathname://../../../assets/Supplementary_table_template.csv) files. These template files also take advantage of [ontologies](/docs/topics/ontology) to unambiguously identify terms for humans and machines. PubChem_CIDs as well as CAS_RN are easily retrieved in [PubChem](https://pubchem.ncbi.nlm.nih.gov/) by searching via SMILES, InChI or InChIKey. Care should be taken for SMILES if inorganic compounds are described. If the repository allows to create a dataset and provides a DOI prior upload of the dataset, the DOI of the dataset should be added to this table.

If the experimental work is documented in an [ELN](/docs/topics/eln), this table includes all the compiled information by the ELN system. Chemotion ELN generates [SMILES](/docs/topics/smiles), [InChI](/docs/topics/inchi), InChIKey as well as RInChI and RInChIKey for compounds and reactions i.e. such a table is not required with structured domain-specific repositories such as Chemotion Repository.
