...
Repositories can be hosted at institutional server or are provided by broader organisations or consortia such as NFDI4Chem. The use of repositories is essential for data deposition according to the [FAIR principles](/docs/fair).
...
In order to allow data reuse by other researchers, [metadata](/docs/metadata) including [provenance information](/docs/provenance/) are required beside the actual data. Metadata describe the content of the research data and provide information about its creation, the methods or software used as well as legal aspects. Metadata can be either added manually or can be provided through other applications. The process to manually add metadata via a metadata editor can be compared to the process of submitting a manuscript to a publisher via the publishers submission system. 
