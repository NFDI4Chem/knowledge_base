...
import {
	...
	repositoryStyle,
	RepoDiv,
} from "@site/src/data/repoCardData";
...
<BulletContainer>
	{repositoryData.map((repo, index) => (
		<BulletBox secondary {...repositoryStyle}>
			<Link to={repo.link} key={index}>
				<RepoDiv img link={repo.link}>
...
<small>
	<em>
		\*Resource which does not accept direct submission of MS data in vendor
		or mzML format, but data in MassBank format, see
