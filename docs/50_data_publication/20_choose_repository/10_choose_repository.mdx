...
import { repositoryData, repositoryStyle } from "./repoCardData";
...
<BulletContainer>
  {repositoryData.map((repo) => (
    <BulletBox key={repo.name} secondary {...repositoryStyle}>
      <FloatImage url={repo.url} alt={repo.name} float="none" />
      {repo.description}
    </BulletBox>
  ))}
</BulletContainer>
