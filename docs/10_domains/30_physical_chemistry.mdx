...
### How to do it [FAIR](/docs/topics/fair)?
- [documentation of all research data](/docs/data/data_documentation) and [metadata](/docs/topics/metadata) is carried out digitally using a suitable repository (e.g. NOMAD, ioChem-BD or a general-purpose repository) to store the input files, main log and structures files (if not included in the log)
...
- simultaneously with [publication](/docs/data/data_publication) as a research article in a scientific journal, the data in the [repository](/docs/topics/repositories) is linked to the article (incl. semantically richly annotated raw and processed data, if possible in open data formats for reuse)
...
### Challenges to make data FAIR
- no standardized transfer into [open file formats](/docs/topics/format_standards) for different simulation packages
