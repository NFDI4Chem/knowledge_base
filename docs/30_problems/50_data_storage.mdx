...
page_id: data_storage
related_pages:
	problems: [data_organisation, data_documentation]
	topics: [repositories]
---

If you plan to collect data and process it into information, you should consider different types of storage with regard to security, backup, access time and sharing with others. Also interesting is further [how to estimate the computational resources for data processing and analysis](https://rdmkit.elixir-europe.org/storage.html#how-do-you-estimate-computational-resources-for-data-processing-and-analysis). There are different requirements for the entire [Data Life Cycle](/docs/topics/data_life_cycle/). With regard to the workflows used in the project, care should also be taken to secure these workflows and tools (software version!) to ensure the reproducibility of results.
...
When planning [data management](/docs/problems/dmp/), think about storage solutions and request short-term and long-term storage in advance.

#### Necessary requirements when designing a storage system:
- space requirements for collection or generation raw data including temporary files ("fast storage")
- space requirements for data that can be permanently accessed over the duration of the project
- access to the data (in case of collaborative projects), how do they expect to access the data and for what purpose
- transfer speed
...
- keeping track of the changes (version control), conflict resolution, data mentoring and back-tracing capabilities
...
- and there should be **0** problems when recovery. (So test your backups regularly...)
...
Many of the requirements are often solved by dedicated [repositories](/docs/topics/repositories/). It is also worth taking a look at group drives or cloud services such as NextCloud (on-premises). Your local IT team will help you with services that they normally support. But nevertheless: Make sure to generate good documentation (i.e., README file) and metadata together with the data. Check if your institute provides a (meta)data management system, such as iRODS, DataVerse, FAIRDOM-SEEK or OSF.
